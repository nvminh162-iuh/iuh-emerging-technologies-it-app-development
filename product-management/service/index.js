...
const save = async (id, body, file) => {
    const { name, customer, price, quantity, category } = body; // trừ image
    // payload create
    let item = { id: id ?? randomUUID(), name, customer, price, quantity, category } // trừ image
    // payload update
    if (id) {
        const res = await findById(id);
        if (!res) throw new Error("Item not found");
        item = { ...res, name, customer, price, quantity, category }; // đã rải url ảnh cũ vào
    }
    // payload s3
    if (file) {
        // upload file
